            ...
            <div class="col-md-4"  >
              <div class="product-item" style="border:1px blue dotted; width:manual;" >
                    ...
                    <div class="down-content"  style="text-align:right;">
                      ...
                      <a href="#"> <h6 > مادة {{$teachers->teacher_Major}}    <strong class="" ></strong>  </h6></a>
                  ...
                  {{-- <p>{{$teachers->teaching_level}}<strong class="bi bi-cash"></strong>  </p> --}}
                      ...
                      {{-- فورم للبحث  للحجوزات --}}
                      <form action="{{url('addcart',$teachers->id)}}" method="POST" style="text-align: center"

                        >
                        @csrf
                        <div >
                          <div dir="rtl" style="padding:7px; " >

                    <label >اختر الوقت المناسب للحصة <p  ></p></label>
                    {{-- //  هاي ضفناها حتى ما يحجز تاريخ قديم
                      min="{{now()->format('Y-m-d\TH:i')}} " --}}
                    <input type="datetime-local"  name="time" min="{{now()->format('Y-m-d\TH:i')}}" required="" >


                  </div>
                  </div>
                  <div>
                    <input  class="btn btn-primary" style="margin:5px;background-color:#007BFF;align-items:center;" type="submit" value="احجز الان"  >
